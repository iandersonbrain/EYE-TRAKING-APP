...
export default function AlgorithmTrendsTracker({ onSyncComplete }: AlgorithmTrendsTrackerProps) {
  ...
  return (
    <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-2xl border border-slate-800 space-y-6 relative overflow-hidden">
      ...
      {/* Educational Guide Box for Algorithmic Monitor */}
      <div className="bg-slate-800/90 rounded-2xl p-4 border border-cyan-500/30 text-slate-200 space-y-2">
        <div className="flex items-center space-x-2 font-bold text-xs text-cyan-300">
          <Info className="w-4 h-4 text-cyan-400 shrink-0" />
          <span>Manual de Uso: ¿Cómo interpretar y aprovechar el Monitor Algorítmico Live?</span>
        </div>
        <p className="text-xs text-slate-300 leading-relaxed">
          <strong>• ¿Qué es esta herramienta?</strong> Es un radar en vivo que te mantiene al día sobre cómo evalúan los algoritmos de <strong>Meta Ads</strong>, <strong>Google Display</strong>, <strong>TikTok</strong> y <strong>LinkedIn</strong> tus anuncios.<br/>
          <strong>• ¿Por qué es crucial?</strong> Si publicas un anuncio sin cumplir estas reglas (ej. poner texto donde Instagram coloca su botón de mensaje o saturar un banner de Google), el algoritmo <strong>sube tus costos de subasta (eCPM)</strong> o <strong>reduce el alcance de tus campañas</strong>.<br/>
          <strong>• ¿Cómo usarlo?</strong> Elige abajo la red publicitaria que vas a utilizar (Meta, Google, TikTok o LinkedIn). Revisa los <strong>Factores Clave (con sus % de peso)</strong> y la lista de <strong>Disparadores de Penalización</strong> para ajustar tu diseño antes de lanzar la pauta.
        </p>
      </div>
